RSpec.describe GeetestRubySdk::Validator do
  ...
  describe 'local validate' do
    it 'returns success' do
      ...
      expect(geetest_validator.local_valid?(challenge, validate)).to be true
    end

    it 'returns false if validate is invalid' do
      expect(geetest_validator.local_valid?(challenge, validate)).to be false
    end
  end
  ...
  describe 'validate' do
    ...
    it 'raises an error when response is not a json' do
      ...
      expect { geetest_validator.valid?(challenge, validate, seccode) }.to raise_error('validate request failed')
    end
    ...
    it 'returns true if all params are valid' do
      stub_request(:post, 'http://api.geetest.com/validate.php')
        .with(body: request_body)
        .to_return(body: response_body, status: 200, headers: response_headers)
      expect(geetest_validator.valid?(challenge, validate, seccode)).to be true
    end
  end
end
